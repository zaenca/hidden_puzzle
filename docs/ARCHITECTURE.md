...
> ## Внимание: core игры сменился на Sort
>
> Документ ниже описывает **исходную** концепцию: гибрид «пазл → раскрытие →
> поиск предметов». Она реализована, работает и осталась в проекте как
> `mode: "legacy"`, но **новым основным core стал Sort**, и разделы 1, 4.2–4.4
> про порядок фаз уровня к новому контенту больше не относятся.
>
> Что верно и сегодня: граница между core и метой (раздел 1, таблица), правила
> данных (`ContentDB` ленив, контент — JSON), `MetaService` как единственное
> место, где меняется мир, `EffectRunner`, `TaskResolver`, сохранение.
>
> **Sort в двух абзацах.** Игрок тапает по предмету на кадре, предмет уходит в
> лоток на семь ячеек; три предмета одной смысловой категории закрываются сами
> и освобождают ячейки. Полный лоток без закрытой группы — проигрыш и мгновенный
> перезапуск той же раскладки. Разобранное поле — конец уровня.
>
> **Что поменялось в коде.** Экран уровня (`Screen.LEVEL`) ведёт теперь на
> `core/level/level_controller.tscn` — маршрутизатор: он читает `mode` из
> `LevelDefinition` и создаёт игровой модуль по `GameplayRegistry`. Контракт
> модуля — `core/level/gameplay_module.gd`: ему дают `LevelContext`, он
> однажды отдаёт `LevelResult`. Прежний `hybrid_level.gd` стал одним из таких
> модулей (`mode: "legacy"`) и не изменился ни в чём, кроме приёма контекста.
>
> Sort живёт в `core/sort/`: `sort_state.gd` — правила без единой ноды (их же
> проверяет валидатор и проходит headless-прогон), `sort_solver.gd` —
> доказательство проходимости и путь прохождения, `sort_module.gd` — экран,
> ввод и анимации. Данные — `data/sort_definition.gd` (+ категория и экземпляр
> предмета), читаются `ContentParser` из секции `sort` в JSON уровня.
>
> Обучение Sort — событийное: шаги лежат в `content/tutorial/<id>.json` и
> привязаны к событиям уровня (`level_started`, `first_item_picked`, …), а не к
> его id.
