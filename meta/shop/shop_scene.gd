...
func _interact(slot_id: String) -> void:
	...
	## Тап мог закрыть задачу, а её действие — попросить сцену. Спрашиваем об
	## этом здесь: очередь диалогов сама себя не разбирает, а ждать ближайшего
	## пройденного уровня разговор о том, что игрок сделал только что, не может.
	Game.play_pending_dialog()
