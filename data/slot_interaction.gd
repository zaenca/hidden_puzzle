...
##
## Флаг считается движением наравне с новым состоянием и выданным предметом:
## по флагам закрываются задачи и заводятся разговоры, и правило, которое
## ставит флаг, для игрока — такое же дело, как повёрнутый в замке ключ. Без
## этого запертая дверь, единственный смысл которой «попробуй меня», осталась
## бы неотличимой от нарисованной на фоне.
func is_progress() -> bool:
	return not set_state.is_empty() or not grant_item.is_empty() \
		or not set_flag.is_empty() or not once_flag.is_empty()
