...
## С каким состоянием локация появляется в новой партии. "locked" — закрыта на
## карте и недоступна; всё остальное означает «сюда уже можно».
##
## Поле данных, а не эффект действия, намеренно. Первый объект района открыт
## игроку с самого начала — это факт о мире, а не награда за работу, и
## изображать его фиктивным уровнем ради права выставить флаг значит платить
## геймплеем за строчку состояния.
